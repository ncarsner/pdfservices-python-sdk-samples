...
#
# This sample illustrates how to generate an accessibility check report of input PDF file.
#
...
class PDFAccessibilityChecker:
    ...
    def create_pdf_output_file_path(self) -> str:
        ...
        os.makedirs("output/PDFAccessibilityChecker", exist_ok=True)
        return f"output/PDFAccessibilityChecker/accessibilitychecker{time_stamp}.pdf"

    def create_json_output_file_path(self) -> str:
        now = datetime.now()
        time_stamp = now.strftime("%Y-%m-%dT%H-%M-%S")
        os.makedirs("output/PDFAccessibilityChecker", exist_ok=True)
        return f"output/PDFAccessibilityChecker/accessibilitychecker{time_stamp}.json"
